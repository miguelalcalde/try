...
- **With `TRY_GH_ROOT` set**: For `github.com` URLs, creates directory at `$TRY_GH_ROOT/<owner>/<repo>` (or `$TRY_GH_ROOT/<owner>/<custom_name>` if custom name provided). If the directory already exists, just `cd`s into it without cloning.
- Clones repository into that directory (unless it already exists when using `TRY_GH_ROOT`)
...
TRY_GH_ROOT=~/github try clone https://github.com/user/repo myproject
...
| `TRY_GH_ROOT` | If set, GitHub repositories are cloned to `$TRY_GH_ROOT/<owner>/<repo>` instead of date-prefixed directories under tries path |

`GH_PATH` is intentionally ignored. GitHub CLI uses that variable for the path to the `gh` binary.
